...
!  public  ::  Stomatal_Resistance_lu_tr
  ...
  real, allocatable     ::  vd_nox_fac(:,:)  ! (nx,ny)
...
contains
  ...
  subroutine LE_DryDepos_Init( rcF, status )
    ...
    ! read factor:
    call ReadRc( rcF, 'deposition.adhoc.factor.nox', factor, status )
    IF_NOTOK_RETURN(status=1)
    ! info ...
    write (gol,'("ADHOC deposition factor for nox: ",f12.6)') factor; call goPr
    ! fill:
    vd_nox_fac = factor
    ...
    ! info ...
    write (gol,'("")'); call goPr
    write (gol,'("deposition scheme per tracer:")'); call goPr
    write (gol,'("------ --------------- ----- --------------- ----")'); call goPr
...
!    IF_NOTOK_RETURN(status=1)
    ...
    do k = 1, nspec
      ...
      do lu = 1,nlu
        ...
        !$OMP parallel
        !$OMP   workshare
        bud_ex_now_lu(:,:,k,lu) = - (c(:,:,1,k) - ccomp_tot_lu(:,:,k,lu)) * &
                                   (1.0 - exp(-vd_lu(:,:,k,lu)*dt_dh ))*dh
        !$OMP   end workshare
        !$OMP end parallel
        ...
        !$OMP parallel
        !$OMP   workshare
        where (bud_ex_now_lu(:,:,k,lu) <= 0.0)
          bud_ex_now    (:,:,k   ,ex_drydepo) = bud_ex_now(:,:,k,ex_drydepo) + &
                                                bud_ex_now_lu(:,:,k,lu)*lu_fracs(:,:,lu)
        elsewhere
          bud_ex_now    (:,:,k   ,ex_dryemis) = bud_ex_now(:,:,k,ex_dryemis) + &
                                                bud_ex_now_lu(:,:,k,lu)*lu_fracs(:,:,lu)
        endwhere
        !$OMP   end workshare
        !$OMP end parallel
        ...
        if ( update_bud ) then
          ...
          !$OMP   workshare
          where (bud_ex_now_lu(:,:,k,lu) <= 0.0)
            bud%ex_hour_lu(:,:,k,lu,ex_drydepo) = bud%ex_hour_lu(:,:,k,lu,ex_drydepo) + &
                                                  bud_ex_now_lu(:,:,k,lu)
          elsewhere
            bud%ex_hour_lu(:,:,k,lu,ex_dryemis) = bud%ex_hour_lu(:,:,k,lu,ex_dryemis) + &
                                                  bud_ex_now_lu(:,:,k,lu)
          endwhere
          !$OMP   end workshare
          !$OMP end parallel
      ...
      if ( update_bud ) then
        ...
        bud%ex_hour(:,:,k,:) = bud%ex_hour(:,:,k,:) + bud_ex_now(:,:,k,:)
      end if  ! update budgets

      ! Compute new concentration, add contribution of all exchange processes:
      c(:,:,1,k) = c(:,:,1,k) + sum(bud_ex_now(:,:,k,:),3) * inv_dh
  ...
  subroutine LE_DryDepos_Setup_vd( c, bud, only_if_depends_on_c, t, status )
    ...
    use Dims                      , only : nx, ny, nz, nspec
    use Dims                      , only : ustar, monin, lst,coszen
    use Dims                      , only : outF, runF
    use indices
    ...
    use LE_Landuse_Data           , only : ustar_lu
    use LE_Landuse_Data           , only : z0m_lu, z0h_lu
    ...
    if ( any( spectype_depos == SPECTYPE_DEPOS_AERO_ZHANG ) &
      ...
      do i_aerosol = 1, n_aerosol
        ...
        vs(:,:,ispec) = slinnfac(ispec) * Sedimentation_Velocity( rhopart(ispec), &
                                                        partsize(:,:,1,i_aerosol), &
                                                        slipcor (:,:,1,i_aerosol), &
                                                        viscos(:,:,1) )
    ...
    do ispec = 1, nspec
      ...
      select case ( spectype_depos(ispec) )
        ...
        ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          !$OMP   shared( rh, radd, hpres, temp, sst, monin, ustar, coszen ) &
          !$OMP   shared( lli ) &
          !$OMP   shared( dens) &
          !$OMP   shared( bud ) &
          !$OMP   shared( lu_fracs ) &
                ...
                if ( ispec == i_o3 ) then
                  ...
                  if ( extra_output .or. with_ozone_specials ) then
                    ...
                    call DryDepos_Gas_DEPAC( specnam1, iday, lat, ts, ust, glrad, coszen1, &
                             rh_surf, lai(i,j,k), sai(i,j,k), nwet, k, iratns, &
              ...
              if ( partsize(i,j,1,i_aerosol) > 0.0  ) then
                ...
                do k = 1, nlu
                  ...
                  call LE_DryDepos_Aero_Zhang_vd( vd_lu(i,j,ispec,k), Rs(i,j,ispec,k), &
                                   ...
                                   k, Ra_h_htop_z0_lu(i,j,k), ustar_lu(i,j,k), &
                                   status )
      ...
      ! Andres
      if ( (ispec == ispec_no2) .or. (ispec == ispec_no) ) then
        ! loop over land use classes:
        do k = 1, nlu
          ! apply factor:
          vd_lu(:,:,ispec,k) = vd_lu(:,:,ispec,k) * vd_nox_fac
        end do
      end if
  ...
  subroutine mix2canopytop( c, czcanopytop_o3_lu )
    ...
    real, intent(in)    :: c(nx,ny,nz)             ! atmospheric concentration oz ozone
